...
class UploadBehavior extends ModelBehavior {
	...
	protected $_pathMethods = array (
			'flat',
			'primaryKey',
			'random',
			'randomCombined' 
	);
	protected $_resizeMethods = array (
			'imagick',
			'php' 
	);
	 ...
	 * @param array $options
	 *        	array of configuration settings for a field
	 * @return void
	 */
	...
	public function beforeSave(Model $model, $options = array()) {
		...
		foreach ( $this->settings [$model->alias] as $field => $options ) {
			...
			$this->runtime [$model->alias] [$field] ['name'] = $this->_retrieveName ( $model, $field, $this->runtime [$model->alias] [$field] ['name'], $model->data, array (
	...
	public function isFileUploadOrHasExistingValue(Model $model, $check) {
		if (! $this->isFileUpload ( $model, $check )) {
			...
			if (! empty ( $model->data [$model->alias] [$pkey] )) {
				$field = $this->_getField ( $check );
				$fieldValue = $model->field ( $field, array (
						$pkey => $model->data [$model->alias] [$pkey] 
				) );
	 ...
	 *        	Configured name
	 * @param string $usePrefixStyle
	 *        	Whether to use prefix style or not
	 * @return string
	 */
	 ...
	 *        	Model instance
	 * @param array $data
	 *        	array containing data to be saved to the record
	 * @return void
	 */
	 ...
	 *        	Name of field being modified
	 * @param string $path
	 *        	Path to existing file on disk
	 * @param string $size
	 *        	Name of size to use
	 * @param string $geometry
	 *        	Dimensions for current size
	 ...
	 * Code from http://www.roscripts.com/snippets/show/55
	 *
	 * @param resource $img
	 *        	An image resource, such as one returned by imagecreatefromjpeg()
	 * @param string $type
	 *        	'horz' or 'vert'
	...
	protected function _grab(Model $model, $field, $uri) {
		...
		$file = $socket->get ( $uri, array (), array (
				'redirect' => true 
		) );
	 ...
	 *        	Name of field being modified
	 * @param array $options
	 *        	Options to use when building a path
	 * @return string
	 *
	 */
	protected function _path(Model $model, $field, $options = array()) {
		...
		if (! $options ['isThumbnail']) {
			$options ['path'] = str_replace ( array (
					'{size}',
					'{geometry}' 
			), '', $options ['path'] );
		}
	 ...
	 *        	Name of field being modified
	 * @param array $params
	 *        	Array of parameters to use for the thumbnail
	 * @return string
	 *
	 */
	...
	protected function _prepareFilesForDeletion(Model $model, $field, $data, $options = array()) {
		...
		if (! empty ( $options ['fields'] ['dir'] ) && isset ( $data [$model->alias] [$options ['fields'] ['dir']] )) {
			...
			if (in_array ( $options ['pathMethod'], array (
					'_getPathFlat',
					'_getPathPrimaryKey' 
			) )) {
				$model->id = $data [$model->alias] [$model->primaryKey];
				$dir = call_user_func ( array (
						$this,
						'_getPath' 
				), $model, $field );
			} else {

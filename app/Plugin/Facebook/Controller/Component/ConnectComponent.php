...
class ConnectComponent extends Component {
	 ...
	 * If false, you can use $this->hasAccount
	 ...
	 *
	 * @param
	 *        	controller object to attach to
	 * @param
		...
		if ($Auth->user ( 'id' )) {
			...
			elseif (empty ( $this->authUser ) && $this->createUser) {
				...
				if ($this->__runCallback ( 'beforeFacebookSave' )) {
					$this->hasAccount = ($this->User->save ( $this->authUser, array (
							'validate' => false 
					) ));
				} else {
					$this->authUser = null;
	 ...
	 *
	 * @param
	 *        	field key to return (xpath without leading slash)
	 * @param
	 *        	mixed return
	 */
	...
	/**
	 * Get the developer roles for the application.
	 * Returns an object
	 * Example return:
	 * object(stdClass)[50]
	 * public 'data' =>
	...
	public function roles() {
		...
		$roles = json_decode ( file_get_contents ( 'https://graph.facebook.com/' . $app_id . '/roles?' . $app_access_token ) );
	 ...
	 *        	string callback
	 * @param
	 *        	mixed passed in publiciable (optional)
